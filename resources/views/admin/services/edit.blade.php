...
<div class="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
    ...
    <!-- main form -->
    <form action="{{ route('service.update', $service->id) }}" method="POST" class="space-y-6 bg-white shadow-sm rounded-xl p-6 md:p-8 border border-slate-200" enctype="multipart/form-data">
        @csrf
        <!-- ====== heading (plain) ====== -->
        <div>
            <label for="heading" class="block text-sm font-medium text-slate-700 mb-1">Heading</label>
            <input type="text" id="heading" name="heading" placeholder="Service main heading" value="{{ old('heading', $service->heading) }}"
        ...
        <div>
            ...
            <input type="text" id="small_pag" name="small_pag" placeholder="Short description" value="{{ old('small_pag', $service->small_pag) }}"
                   class="w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-2.5 border">
            @error('samll_pag')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <input type="text" name="first_heading" placeholder="Service First heading" value="{{ old('first_heading', $service->first_heading) }}"
                   class="w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-4 py-2.5 border">
            @error('first_heading')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        ...
        <div>
            ...
            <select name="serviceCat_id" id="serviceCat_id"
                ...
                @foreach ($serCat as $category)
                    <option value="{{ $category->id }}" {{ old('serviceCat_id', $service->serviceCat_id) == $category->id ? 'selected' : '' }}>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
