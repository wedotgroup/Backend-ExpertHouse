...
@section('content')
    ...
    <div class="w-full max-w-7xl mx-auto bg-white shadow-2xl rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <form action="{{ route('insight.update', $post->id) }}" method="POST" enctype="multipart/form-data" class="px-8 py-6"
            id="postForm">
            @csrf
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">
                            <img src="{{ asset($post->image) }}" alt="" width="90">
                        </p>
                        @error('image')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div class="mt-6">
                ...
                <div class="ckeditor-wrapper">
                    <textarea name="paragraph" id="paragraph" rows="8" placeholder="Write your paragraph content here..."
                        class="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 outline-none transition-all bg-white shadow-sm @error('paragraph') border-red-500 @enderror">{{ old('paragraph', $post->paragraph ?? '') }}</textarea>
                </div>
                @error('paragraph')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mt-6">
                ...
                <div class="ckeditor-wrapper">
                    <textarea name="description" id="description" rows="8" placeholder="Write your description here..."
                        class="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 outline-none transition-all bg-white shadow-sm @error('description') border-red-500 @enderror">{{ old('description', $post->description ?? '') }}</textarea>
                </div>
                @error('description')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
